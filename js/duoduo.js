  ...
  if (downloadUrl) {
   ...
   async function searchContent(keyword) {
       try {
           ...
           const url = `https://tv.yydsys.cc/index.php/vod/search.html?wd=${encodedKeyword}`;
   ...
   async function categoryContent(tid, pg = 1, extend) {
    ...
    let url = `https://tv.yydsys.cc/index.php/vod/show/area/{area}/by/{by}/class/{cateId}/id/${tid}/page/${pg}/year/{year}.html`;
    // 替换 URL 中的占位符
    if (extendObj) {
...
async function detailContent(ids) {
  const url = `https://tv.yydsys.cc${ids}`;
  try {
    //console.log(url);
